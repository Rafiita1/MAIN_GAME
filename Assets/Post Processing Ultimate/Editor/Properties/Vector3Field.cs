...
namespace NTEC.PPU{
	public class Vector3Field : VisualProperty{
		...
		public override void Show(){
			...
			EditorGUI.LabelField(new Rect(position.x, position.y, position.width, 20), new GUIContent(name, "Vector3 property"), style[0]);
			...
			Values[2] = EditorGUI.FloatField(new Rect(position.x + 16, (position.y + 3 * (20) + 2), 54, 16), float.Parse(Values[2], CultureInfo.InvariantCulture), style[3]).ToString();
			Values[3] = EditorGUI.FloatField(new Rect(position.x + 86, (position.y + 3 * (20) + 2), 54, 16), float.Parse(Values[3], CultureInfo.InvariantCulture), style[3]).ToString();
			Values[4] = EditorGUI.FloatField(new Rect(position.x + 156, (position.y + 3 * (20) + 2), 54, 16), float.Parse(Values[4], CultureInfo.InvariantCulture), style[3]).ToString();
		}
	}
}