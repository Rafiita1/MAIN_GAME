...
namespace NTEC.PPU{
	public class IntField : VisualProperty{
		...
		public override void Show(){
			...
			EditorGUI.LabelField(new Rect(position.x, position.y, position.width, 20), new GUIContent(name, "Int property"), style[0]);
			...
			Values[2] = EditorGUI.IntField(new Rect(position.x + 2, (position.y + 3 * (20) + 2), position.width - 4, 16), int.Parse(Values[2]), style[3]).ToString();
		}
	}
}