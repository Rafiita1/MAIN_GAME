...
namespace NTEC.PPU{
	public class GradientNoise : VisualElement{
		...
		public override void ShowLabels(){
			...
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 2 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("X Value", "X input value"), style[1]);
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 3 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("Y Value", "Y input value"), style[1]);
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 4 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("XY", "Input value"), style[1]);
		}
	}
}