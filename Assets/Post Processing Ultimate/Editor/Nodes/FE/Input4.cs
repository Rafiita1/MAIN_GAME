...
namespace NTEC.PPU{
	public class Input4 : VisualElement{
		...
		public override void ShowLabels(){
			...
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 5 * (40) + 8) * scale, (position.width - 8) * scale, 32 * scale), new GUIContent("W Value", "Input W"), style[2]);
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 6 * (40) + 8) * scale, (position.width - 8) * scale, 32 * scale), new GUIContent("XYZW", "Input XYZW"), style[2]);
		}
	}
}