...
namespace NTEC.PPU{
	public class StartsAtTop : VisualElement{
		...
		public override void ShowLabels(){
			...
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 1 * (40) + 8) * scale, (position.width - 8) * scale, 32 * scale), new GUIContent("Value", "UNITY_UV_STARTS_AT_TOP"), style[2]);
		}
	}
}