...
namespace NTEC.PPU{
	public class VarLoop2 : VisualElement{
		...
		public VarLoop2(){
			position = new Rect(32, 32, 192, 135 + 6 * 40);
			name = "VarLoop2";
		...
		public override void ShowLabels(){
			...
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 5 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("Input", "Input value"), style[1]);
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 6 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("Iterations", "Number of loops"), style[1]);
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 7 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("Start", "Start iterator value"), style[1]);
			Values[2] = EditorGUI.Popup(new Rect(position.x * scale, (position.y + 8 * (40) + 4) * scale, position.width * scale, 32 * scale), Int32.Parse(Values[2]), options1.ToArray(), style[4]).ToString();
		}
	}
}