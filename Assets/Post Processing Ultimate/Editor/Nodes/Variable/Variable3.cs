...
namespace NTEC.PPU{
	public class Variable3 : VisualElement{
		...
		public Variable3(){
			...
			joints[4].prohibitions.Add(7);
			joints[5].prohibitions.Add(7);
			joints[6].prohibitions.Add(7);
			joints[7].prohibitions.Add(4);
		...
		public override void ShowLabels(){
			...
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 5 * (40) + 8) * scale, (position.width - 8) * scale, 32 * scale), new GUIContent("XYZ", "All output values"), style[2]);
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 6 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("X Value", "First input value"), style[1]);
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 7 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("Y Value", "Second input value"), style[1]);
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 8 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("Z Value", "Third input value"), style[1]);
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 9 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("XYZ", "All input values"), style[1]);
		}
	}
}