...
namespace NTEC.PPU{
	public class Variable4 : VisualElement{
		...
		public Variable4(){
			position = new Rect(32, 32, 192, 495);
			name = "Variable4";
		...
		public override void ShowLabels(){
			...
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 7 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("X Value", "First input value"), style[1]);
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 8 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("Y Value", "Second input value"), style[1]);
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 9 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("Z Value", "Third input value"), style[1]);
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 10 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("W Value", "Fourth input value"), style[1]);
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 11 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("XYZW", "All input values"), style[1]);
		}
	}
}