...
namespace NTEC.PPU{
	public class Refract : VisualElement{
		...
		public override void ShowLabels(){
			...
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 2 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("I Value", "A floating-point, ray direction vector"), style[1]);
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 3 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("N Value", "A floating-point, surface normal vector"), style[1]);
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 4 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("n Value", "A floating-point, refraction index scalar"), style[1]);
		}
	}
}