...
namespace NTEC.PPU{
	public class Smoothstep : VisualElement{
		...
		public override void ShowLabels(){
			...
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 2 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("Min", "The minimum range of the x parameter"), style[1]);
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 3 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("Max", "The maximum range of the x parameter"), style[1]);
			EditorGUI.LabelField(new Rect((position.x + 4) * scale, (position.y + 4 * (40) + 8) * scale, (position.width) * scale, 32 * scale), new GUIContent("X Value", "The specified value to be interpolated"), style[1]);
		}
	}
}